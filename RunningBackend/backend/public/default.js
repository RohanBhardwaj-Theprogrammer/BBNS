...
function displayProductDetails(QrDetails) {
  fetch('/api/check-product-genenity', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
    body: JSON.stringify({ QrDetails })
  })
    .then((response) => response.json())
    .then((data) => {
      const productGenuintyDiv = document.getElementById("productGenuinty");
      const manufacturerDetailsDiv = document.getElementById("manufacturerDetails");

      let manufacturerDetailsTable = document.createDocumentFragment();
      const table = document.createElement("table");
      manufacturerDetailsTable.appendChild(table);

      for (let key in data.manufacturerDetails) {
        let row = document.createElement("tr");
        ...
        let valueCell = document.createElement("td");
        valueCell.textContent = data.manufacturerDetails[key];
        row.appendChild(valueCell);
    ...
    .catch((error) => {
      console.error("Error checking product genuineness:", error);
    });
}
